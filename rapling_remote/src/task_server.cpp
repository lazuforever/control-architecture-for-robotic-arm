    ...
    void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<rapling_msgs::action::ArduinobotTask>> goal_handle)
    {
      ...
      if (task == 1)
      {
        ...
        if (!trajectory_points.empty()) {
            ...
            // Publicar el JointState con el vector completo
        ...
        {
          RCLCPP_INFO(get_logger(), "Planeamiento exitoso para posición 1");
          arm_move_group.move();
        }
        else
        {
          RCLCPP_ERROR(get_logger(), "Fallo en el planeamiento para posición 1");
          result->success = false;
          goal_handle->abort(result);
          return;
        }
